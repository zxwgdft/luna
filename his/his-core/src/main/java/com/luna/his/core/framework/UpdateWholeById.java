 ...
 * <p>
 * 当实体类为BaseModel，更新条件增加TenantId，加强数据安全
 *
 * @auther TontoZhou
 * @since 2018-04-06
 */
public class UpdateWholeById extends AbstractMethod {
    ...
    public MappedStatement injectMappedStatement(Class<?> mapperClass, Class<?> modelClass, TableInfo tableInfo) {
        ...
        if (BaseModel.class.isAssignableFrom(tableInfo.getEntityType())) {
            ...
            sql = String.format(formatSql, tableInfo.getTableName(),
                    sqlSet2(tableInfo.isWithLogicDelete(), false, tableInfo, false, ENTITY, ENTITY_DOT, true),
            ...
            final String additional = optlockVersion(tableInfo) + tableInfo.getLogicDeleteSql(true, true);
            sql = String.format(formatSql, tableInfo.getTableName(),
                    sqlSet2(tableInfo.isWithLogicDelete(), false, tableInfo, false, ENTITY, ENTITY_DOT, false),
    ...
    protected String sqlSet2(boolean logic, boolean ew, TableInfo table, boolean judgeAliasNull, final String alias,
                             final String prefix, boolean isBaseModel) {
        /*
         * 从原updateById方法复制而来的代码
         */
        final String newPrefix = prefix == null ? EMPTY : prefix;
        String fieldCreateTime = LambdaUtil.getFieldName(BaseModel::getCreateTime);
        String fieldUpdateTime = LambdaUtil.getFieldName(BaseModel::getUpdateTime);
        String fieldTenantId = LambdaUtil.getFieldName(BaseModel::getTenantId);
        String sqlScript = table.getFieldList().stream()
                .filter(i -> {
                    ...
                    if (isBaseModel) {
                        // 如果是基础类型的字段我们不做更新操作
                        String property = i.getProperty();
                        if (StringUtil.equals(property, fieldCreateTime)
                                || StringUtil.equals(property, fieldUpdateTime)
                                || StringUtil.equals(property, fieldTenantId)) {
                            return false;
                        }
                    }
                    return true;
